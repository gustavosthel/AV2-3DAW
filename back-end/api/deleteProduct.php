...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $id = trim($data['id'] ?? ''); // ID do produto a ser excluído

    // Verifica se o ID foi fornecido
    if (empty($id)) {
        http_response_code(400);
        echo json_encode(["erro" => "ID do produto é necessário para a exclusão."]);
        $conn->close();
        exit;
    }

    // Prepara a consulta SQL para exclusão do produto
    $comandoSQL = "DELETE FROM tb_product WHERE id_product = ?";
    ...
    if ($stmt->execute()) {
        if ($stmt->affected_rows > 0) {
            ...
            http_response_code(400);
            echo json_encode(["erro" => "Nenhum produto encontrado com esse ID."]);
        }
    } else {
        http_response_code(500);
        echo json_encode(["erro" => "Erro ao excluir o produto: " . $stmt->error]);
    }
